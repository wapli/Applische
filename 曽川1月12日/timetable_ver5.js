...
/*時間割表色付けここまで--------------------------------曽川*/
	...
	for(var i=0;i<42;i++){
		...
		//document.getElementById("a-"+i.toString()+"p").innerText=localStorage.getItem('place'+i.toString());
	}
				...
				// 				var memoareaContent=document.getElementById("memoarea").value;
				// 				// var memoSaveButton=document.getElementById("memoSaveButton");
				// 				// console.log(memoSaveButton+"ガチでnullやめて");
				// 				for(i=0;i<=10;i++){
				// 					console.log(i+"周目")
								
				// 				//localstorageからnullが返ってくるまでfor文を回す
				// 				if(localStorage.getItem("memoContent"+pageNum.toString()+"-"+i.toString())==null){break;}
								
				// 				}
				// 				localStorage.setItem("memoContent"+pageNum.toString()+"-"+i.toString(),memoareaContent);
				// 				console.log(localStorage.getItem("memoContent"+pageNum.toString()+"-"+i.toString())+":ローカルストレージに保存したメモの内容");

				// 											// pタグを作成(これは新しくメモのボックスを作るものです)
				// 				var memoBox = document.createElement("p");
								
				// 				// id属性に値「"memoBox"+pageNum.toString()+"-"+i」を追加
				// 				memoBox.setAttribute("id", "memoBox"+pageNum.toString()+"-"+i.toString());
				// 				var memocontent=localStorage.getItem("memoContent"+pageNum.toString()+"-"+i.toString())
				// 				// 中身のテキストを設定
				// 				memoBox.innerHTML = memocontent;
								
				// 				var memoSet=document.getElementById("memoSettingSpace");
								
				// 				//メモセットのスペースにメモのpタグの追加
				// 				memoSet.appendChild(memoBox);
				// 			},false);
				// },false);
						...
						// if(memoSaveButton===null||memoSaveButton===undefined){
						// var memoSaveButton=document.getElementById("memoSaveButton");
						// console.log(memoSaveButton+"：初めてのセーブボタン読み込みのはず");
						// saveCount++;
						// }
						// var memoSaveButton=document.getElementById("memoSaveButton");
						// console.log(memoSaveButton+"：頼む！！！");
							
						
						// console.log(document.getElementById("memoarea"));
				...
				case 5: //休講情報
					...
					$('#ajax').on('click',function(){
						...
						// Ajax通信が成功した時
						.done( function(data) {
						$('#result').html("<p>科目コード"+data[0].id+"は「"+data[0].kamoku+"」の授業です。</p>場所は"+data[0].place+"で教授名は"+data[0].kyoushi+"で単位数は"+data[0].tani);
						console.log('通信成功');
						console.log(data);

						localStorage.setItem('kamoku-code'+pageNum.toString(), data[0].id);
						localStorage.setItem('lecture-name'+pageNum.toString(), data[0].kamoku);
						localStorage.setItem('place'+pageNum.toString(), data[0].place);
						localStorage.setItem('teacher-name'+pageNum.toString(), data[0].kyoushi);
						localStorage.setItem('tani'+pageNum.toString(), data[0].tani);
