		...
		if (event.target.id = "ons-timetable.html") {
			...
			if(localStorage.getItem('fastFlag')==0){//アプリを実行したときの一回目に実行すること
				for(var i=0;i<42;i++){
					...
					localStorage.setItem('kyouka'+i.toString(),"　　　");
					localStorage.setItem('kyoushi'+i.toString(),'教授名'+i.toString());
				...
				case 0: //出欠
					...
					//回数を表示させる場所を取得している
						...
						attendance.addEventListener("click", () => {//出席ボタンを押したときの処理
							localStorage.setItem('attendanceDate'+attendanceCountNum.toString(),Year + '年' + Month + '月' + hizuke + '日' + Hour +'時' + Min + '分'+ Sec+'秒');
							attendanceCountNum++;
						...
						late.addEventListener("click", () => {//遅刻ボタンを押したときの処理
							localStorage.setItem('lateDate'+lateCountNum.toString(),Year + '年' + Month + '月' + hizuke + '日' + Hour +'時' + Min + '分'+ Sec+'秒');
							lateCountNum++;
						...
						absence.addEventListener("click", () => {//欠席ボタンを押したときの処理
							localStorage.setItem('absenceDate'+absenceCountNum.toString(),Year + '年' + Month + '月' + hizuke + '日' + Hour +'時' + Min + '分'+ Sec+'秒');
							absenceCountNum++;
						...
						absenceCount.addEventListener("click", () => {//出席回数の詳細を表示
							for(var i=0;i<absenceCountNum;i++){
								...
								absenceTime.insertAdjacentHTML ('afterbegin', '<div  style="width:100%;"><button class="swipe-item" style="width:80%;">'+absenceDate+'秒</button><button　class="absenceDateDelete" style="width:20%;">削除</button></div><br>');
					...
					$('#ajax').on('click',function(){
					 ...
					 .done( function(data) {
						...
						for(var i=0;i<data.length;i++){
							document.getElementById("result").insertAdjacentHTML ('afterbegin', '<ons-list-item id=\"result'+i.toString()+'\"　 style=\"background-color:#00bfff\" bordercolor=\"#ffffff\" ><font color=\"#ffffff\"> 科目コード：'+data[i].id+'　　　　講義名：'+data[i].kyouka+'	　　場所：'+data[i].place+'　　	教授名：'+data[i].kyoushi+'	　　単位数：'+data[i].tani+'</font></ons-list-item><br>');
						...
						for(let count=0;count<data.length;count++){
						  ...
						  localStorage.setItem('kyouka'+pageNum.toString(), data[count].kyouka);
						  localStorage.setItem('kyoushi'+pageNum.toString(), data[count].kyoushi);
